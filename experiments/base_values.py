...
them:
  ...
  2. SIGMA_M / SIGMA_P as used here (0.2625 / 3600 uS/mm) differ from the
     production hrdayapy config.py CPL_SIGMA_M / CPL_SIGMA_P values quoted in
     the manuscript's Table 4 (6.5625e-5 / 0.9 uS/mm) by an exact factor of
     4000 -- consistent with the unit-conversion bug already flagged against
     config.py. Since config.py has since been deleted, THIS file's values
     (validated by producing physiologically plausible CVs in Experiment 2:
     73-79 cm/s myocardial, 249-258 cm/s Purkinje) are treated as the
     ground truth going forward. If a recovered copy of config.py's
     production values is ever reconciled against these, update here and
     nowhere else.
...
# =============================================================================
# Geometry (idealised single-fibre + slab benchmark; shared by Exp 2, 3, 4)
# =============================================================================
P_ROOT  = np.array([5.0, 15.0, -45.0])   # proximal, elevated, stimulus site
P_GRAZE = np.array([5.0, 13.0,  -3.0])   # grazes 3 mm above the slab's top face
P_PMJ   = np.array([5.0,  5.0,   0.0])   # terminal PMJ, centre of the proximal (z=0) face
...
# -----------------------------------------------------------------------------
# These are the EXPERIMENT-VALIDATED values (Experiment 2 produced
# physiologically plausible CVs -- 73-79 cm/s myocardial, 249-258 cm/s
# Purkinje -- using exactly these numbers). They are NOT currently reconciled
# against hrdayapy's production config.py (deleted; last known value showed
# an unexplained 4000x discrepancy on both SIGMA_M and SIGMA_P -- see
# module docstring). Treat these as ground truth until that is resolved.
# =============================================================================
SIGMA_P = 3.6        # mS/mm   Purkinje axial conductivity
SIGMA_M = 2.625e-4    # mS/mm   myocardial bulk conductivity
CM      = 0.01        # uF/mm^2 specific membrane capacitance (PDE)
A_M     = 0.14        # mm^-1   myocardial surface-to-volume ratio
R_P     = 0.01        # mm      Purkinje fibre radius
A_P     = 2.0 / R_P   # mm^-1   Purkinje surface-to-volume ratio

C_PMJ = 5e-2   # mS   lumped PMJ conductance (production default; swept
               #      directly in Experiment 4)
N_PMJ = 4      # nearest myocardial nodes coupled per PMJ (production
