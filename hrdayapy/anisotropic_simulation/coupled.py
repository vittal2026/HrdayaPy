"""
anisotropic_simulation/coupled.py
====================================
Compute/load pair for the ANISOTROPIC coupled Purkinje + myocardium
monodomain simulation -- otherwise identical to simulation/coupled.py
(see purkinje_myocardium_anisotropic_solver.py's module docstring for
exactly what differs). The only interface change is sigma_M -> sigma_l +
sigma_t + fibre_dir.

Two outputs come out of one run:
  1. The main coupled result (surface + Purkinje branch voltage traces,
     used for animation) -- save_path / load_anisotropic_coupled.
...
Note: load_anisotropic_coupled's return shape mirrors the saved .npz
(time, comp_nodes, comp_edges, surf_verts, surf_faces, vert_to_surf_node,
surf_frames, bmap_*, branch_*) -- this is NOT the same dict shape that
compute_anisotropic_coupled returns directly in memory (the solver's own
richer "results" dict). If you're chaining straight from
compute_anisotropic_coupled, use its return value directly;
load_anisotropic_coupled is for starting midway from a saved file.
"""
...
from .functions import purkinje_myocardium_anisotropic_solver
from .._spacing_resolve import resolve_voxel_size


def compute_anisotropic_coupled(
    nodes, elements, act_times, S, Z, phi, fibre_dir,
    *,
    anatomy=None,
    stim_protocol=None,
    ectopic_region=None,
    ...
    dx_p: float = 0.2,
    sigma_P: float = 0.45,
    sigma_l: float = 3.0 * 1.31e-4,   # myocardial conductivity ALONG fibre [mS/mm]
    sigma_t: float = 1.31e-4,          # myocardial conductivity ACROSS fibre [mS/mm]
    Cm: float = 0.01,
    A_M: float = 140.0,   # mm^-1 -- corrected from a stale 0.14 (1000x too
                           # small); matches Table 3, Niederer et al. 2011
    R_P: float = 0.015,   # A_P is derived internally as 2.0/R_P -- no
                           # longer a separate parameter, see the solver's
                           # own [FIX M2] comment for why
    dt: float = 0.1,
    T: float = 400.0,
    theta: float = 0.5,
    ...
    """
    Run the anisotropic coupled Purkinje + myocardium monodomain simulation.
    ...
    fibre_dir : (Nx,Ny,Nz,3) float -- unit fibre direction cosines, RAW
                array-axis order, as returned by hrdayapy.fibres.
                compute_fibres / load_fibres. Pass straight through --
                no axis manipulation needed (see the solver's own
    ...
    sigma_t   : myocardial conductivity ACROSS the local fibre direction [mS/mm]
                ...
                pass hrdayapy.simulation's sigma_M value for both to
                reconcile with an existing isotropic run)
    (remaining physics/solver parameters -- see
    purkinje_myocardium_anisotropic_solver's own docstring for full
    detail; defaults here match its)
    ...
    Returns
    -------
    results : dict -- the solver's raw in-memory result (times,
    ...
    return purkinje_myocardium_anisotropic_solver(
        nodes=nodes, elements=elements, activation_times=act_times,
        S=S, Z=Z, phi=phi, fibre_dir=fibre_dir,
        voxel_size=voxel_size, dx_p=dx_p, sigma_P=sigma_P,
        sigma_l=sigma_l, sigma_t=sigma_t,
        Cm=Cm, A_M=A_M, R_P=R_P, dt=dt, T=T, theta=theta,
        ...
        frame_save_dt=frame_save_dt, anim_tmp_dir=anim_tmp_dir,
...
def load_anisotropic_coupled(path):
    """
    Load a previously saved anisotropic coupled-simulation .npz (surface +
    Purkinje branch traces). No recomputation. See module docstring for
    how this differs from compute_anisotropic_coupled's direct return
    value.
    """
    path = Path(path).with_suffix(".npz")
    if not path.exists():
        raise FileNotFoundError(f"No saved anisotropic coupled simulation at {path}")
    z = np.load(path)
