...
------------------------------------------------------------------------------
  ...
  min_upstroke_vel    scalar                     float32  mV/ms, NaN = gate off
  min_downstroke_vel  scalar                     float32  mV/ms, NaN = gate off
...
-------------
    ...
    maps = compute_activation_maps(vm_snapshots, S,
                                    ...
                                    min_upstroke_vel=10.0,   # mV/ms, gates out plateau chatter
...
def _interp_crossings(Vm: np.ndarray, t: np.ndarray, edge_mask: np.ndarray,
    ...
    Also returns the (signed) dV/dt across the bracketing frame interval, so
    callers can gate spurious crossings (numerical chatter, plateau wobble)
    on upstroke/downstroke speed rather than on voltage alone.

    Returns
    -------
    idx_node : (K,) int     -- node index for every crossing found
    t_cross  : (K,) float32 -- interpolated crossing time [ms]
    velocity : (K,) float32 -- signed dV/dt over [t_i, t_ip1] [mV/ms];
               positive for rising edges, negative for falling edges.
    """
    idx_frame, idx_node = np.nonzero(edge_mask)
    if idx_frame.size == 0:
        return idx_node, np.zeros(0, dtype=np.float32), np.zeros(0, dtype=np.float32)
    ...
    denom = V_ip1 - V_i
    denom_safe = np.where(denom == 0, 1e-9, denom)  # guard: shouldn't happen
    frac  = np.clip((threshold - V_i) / denom_safe, 0.0, 1.0)
    t_cross = t_i + frac * (t_ip1 - t_i)

    dt = t_ip1 - t_i
    dt = np.where(dt == 0, 1e-9, dt)
    velocity = (denom / dt).astype(np.float32)

    return idx_node, t_cross.astype(np.float32), velocity


def _filter_by_velocity(idx_node: np.ndarray, t_cross: np.ndarray,
                         velocity: np.ndarray,
                         min_abs_vel: float | None) -> tuple[np.ndarray, np.ndarray]:
    """
    Drop crossings whose bracketing-interval |dV/dt| is below `min_abs_vel`
    (mV/ms). True depolarisation upstrokes and repolarisation downstrokes
    are fast; threshold chatter from a plateau wobble or numerical noise
    sitting right at `threshold` is slow, so this cleanly separates real
    beats from spurious re-crossings even when the raw voltage level is
    crossed multiple times. `min_abs_vel=None` disables the gate.
    """
    if min_abs_vel is None or idx_node.size == 0:
        return idx_node, t_cross
    keep = np.abs(velocity) >= min_abs_vel
    return idx_node[keep], t_cross[keep]
...
class ActivationMaps:
    ...
    min_upstroke_vel:    float | None = None
    min_downstroke_vel:  float | None = None
    source_vm_path:      str = ""
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"min_upstroke_vel={self.min_upstroke_vel!r} mV/ms, "
            f"min_downstroke_vel={self.min_downstroke_vel!r} mV/ms, "
...
def compute_activation_maps(
    ...
    *,
    ...
    min_upstroke_vel : float, optional
        Minimum depolarisation speed [mV/ms], measured as the interpolated
        dV/dt of the snapshot interval bracketing each rising-edge
        crossing. Crossings slower than this are discarded before events
        are counted/padded. Real upstrokes for a TTP06-like model are on
        the order of 100-300 mV/ms, whereas a plateau wobbling back and
        forth across `act_threshold` typically moves at a few mV/ms or
        less -- so a modest gate (e.g. 5-20 mV/ms, tuned to your
        vm_save_dt) removes that chatter without touching real beats.
        None (default) disables the gate.
    min_downstroke_vel : float, optional
        Same idea as `min_upstroke_vel` but for falling-edge (deactivation)
        crossings, compared against |dV/dt|. Repolarisation is much
        slower than depolarisation, so this should be set well below
        `min_upstroke_vel` (or left None) -- an overly strict value here
        will start discarding genuine repolarisation events, not just
        chatter. None (default) disables the gate.
    ...
    Notes
    -----
    `min_event_gap_ms` and the velocity gates address the same symptom
    (extra phantom beats) from different angles and can be combined: the
    velocity gate rejects slow wobble at the source, while the gap filter
    mops up any remaining close-spaced duplicates. If you're seeing N
    beats detected against a known pacing protocol of M << N beats, start
    by plotting Vm(t) at one of the over-counting nodes to see whether the
    extra crossings are fast (real re-excitation / an alternans-like
    beat) or slow (chatter you want to gate out).
    """
    Vm = vm_snapshots.Vm
    ...
    idx_a, t_a, vel_a = _interp_crossings(Vm, t, rising, act_threshold)
    idx_a, t_a = _filter_by_velocity(idx_a, t_a, vel_a, min_upstroke_vel)
    ...
    idx_d, t_d, vel_d = _interp_crossings(Vm, t, falling, deact_threshold)
    idx_d, t_d = _filter_by_velocity(idx_d, t_d, vel_d, min_downstroke_vel)
    ...
    maps = ActivationMaps(
        ...
        min_upstroke_vel   = float(min_upstroke_vel) if min_upstroke_vel is not None else None,
        min_downstroke_vel = float(min_downstroke_vel) if min_downstroke_vel is not None else None,
...
def save_activation_maps(maps: ActivationMaps, path: str | Path) -> None:
    ...
    payload = dict(
        ...
        min_upstroke_vel   = np.float32(maps.min_upstroke_vel if maps.min_upstroke_vel is not None else np.nan),
        min_downstroke_vel = np.float32(maps.min_downstroke_vel if maps.min_downstroke_vel is not None else np.nan),
...
def load_activation_maps(path: str | Path) -> ActivationMaps:
    ...
    maps = ActivationMaps(
        ...
        min_upstroke_vel   = (float(d["min_upstroke_vel"])
                               if "min_upstroke_vel" in d.files and not np.isnan(d["min_upstroke_vel"])
                               else None),
        min_downstroke_vel = (float(d["min_downstroke_vel"])
                               if "min_downstroke_vel" in d.files and not np.isnan(d["min_downstroke_vel"])
                               else None),
