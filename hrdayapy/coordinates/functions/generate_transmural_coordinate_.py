...
def generate_transmural_coordinate(
    ...
    tol            : unused (kept for signature compatibility). The PIELM
                     output weights are now solved directly via
                     torch.linalg.lstsq rather than iterative CG, since H is
                     small and fixed regardless of voxel count — see note
                     at the solve site.
    maxiter        : unused (kept for signature compatibility, see `tol`)
    ...
    A = torch.cat([lap_coll, bc_w * sig_epi, bc_w * sig_endo], dim=0)      # (Nc+Nb, H)
    c = torch.cat([rhs_coll, bc_w * rhs_epi, bc_w * rhs_endo], dim=0)      # (Nc+Nb,)

    # ── Solve for beta ───────────────────────────────────────────────────
    # Form the normal-equations system in float32 (one GEMM, GPU-fast, and
    # the *only* step whose cost scales with the number of collocation/BC
    # rows — H stays small and fixed regardless of voxel count). Then move
    # only the small H x H system to float64 and solve it directly in a
    # single step.
    #
    # This two-stage approach avoids both failure modes we hit previously:
    #   - Iterative CG on A^T A in float32 accumulates rounding error over
    #     thousands of iterations and eventually breaks the positive-
    #     definiteness assumption, producing NaN — even though A, A^T A,
    #     etc. are all individually finite going in (a single direct solve
    #     has no iteration count for that error to accumulate over).
    #   - Direct torch.linalg.lstsq on the full tall A in float64 is
    #     numerically robust but very slow: it performs a QR-type
    #     decomposition whose cost scales with the (large) number of rows,
    #     and it runs in float64, where most GPUs have a fraction of their
    #     float32 throughput.
    # Forming A^T A in float32 keeps the expensive, row-count-dependent
    # part fast; solving the resulting tiny H x H system in float64 is
    # cheap even at low double-precision throughput, and a one-shot solve
    # (LU with pivoting, via torch.linalg.solve) doesn't have CG's
    # iteration-count-dependent error accumulation. A small ridge term
    # guards against the near-singularity that comes from using random,
    # not orthogonalized, hidden features.
    print(f"  [transmural] Solving for {_N_HIDDEN} output weights ...",
          flush=True)
    AtA = (A.T @ A).double()   # (H, H) — cheap regardless of row count
    Atc = (A.T @ c).double()   # (H,)
    ridge = 1e-8 * torch.diagonal(AtA).mean()
    AtA = AtA + ridge * torch.eye(_N_HIDDEN, dtype=torch.float64, device=_dev)
    beta = torch.linalg.solve(AtA, Atc).float()
